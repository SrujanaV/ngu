  ...
  .controller('HomeCtrl', function ($scope, $state, TemplateService, NavigationService, $timeout, $stateParams, $document, $location) {
    ...
    function AllMedia(data) {
      if (data) {
        ...
        //   $scope.allmedia[i] = _.chunk($scope.allmedia[i], 5);
        //   console.log($scope.allmedia);
        // }

        // for mobile slider
        $scope.allmediaxs = data;
        $scope.allmediaxs = _.chunk($scope.allmediaxs, 3);
        // for (var i = 0; i < $scope.allmediaxs.length; i++) {
        //   $scope.allmediaxs[i] = _.chunk($scope.allmediaxs[i], 3);
        // }
        console.log($scope.allmediaxs);
        // console.log($scope.allmedia);
      }
    }
